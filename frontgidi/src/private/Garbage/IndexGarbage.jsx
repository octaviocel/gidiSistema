...
import { EditModalGarbage } from "./EditModalGarbage";

export const IndexGarbage = () => {
  ...
  const traerResiduos = async () => {
    try {
      setLoad(true);
      const datos = await GarbageService.getAll();
      //console.log(datos)
      setData(datos);
    } catch (error) {
      console.log(error);
  ...
  const saveResiduo = async () => {
    try {
      setLoad(true);
      const res = await GarbageService.create(name);
      //console.log(res)
      if (res) {
        traerResiduos();
        ...
        <>
          ...
          <Container sx={{ marginTop: 5 }}>
            ...
            <div className="grid grid-cols-1 gap-4 sm:gap-5 lg:gap-6">
              <Table columns={columns} data={data} funcion={traerResiduos} />
            </div>
          </Container>
        </>
      )}
              ...
              >
                ...
                <td className="gridjs-td">
                  <span>
                    <div className="flex justify-center space-x-2">
                      <button
                        onClick={(e) => handleOpenEdit(e, row.original.id, row.original.name)}
                      ...
                      <button
                        onClick={() => {
                          Swal.fire({
                            title: "Estas seguro de eliminar este Residuo?",
                            ...
                            if (result.isConfirmed) {
                              try {
                                await GarbageService.deleteGarbage(
                                  row.original.id
                                );
